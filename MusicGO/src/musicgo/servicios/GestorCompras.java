...
public class GestorCompras {
    ...
    public boolean comprarProducto(String idUsuarioOAlias, String idProductoONombre) {
        ...
        if (producto instanceof musicgo.modelo.PaqueteTopTen) {
            musicgo.modelo.PaqueteTopTen paquete = (musicgo.modelo.PaqueteTopTen) producto;
            musicgo.modelo.Playlist nuevaPlaylist = new musicgo.modelo.Playlist(paquete.getNombre(), usuario.getNombre());

            for (String idCancion : paquete.getIdsCanciones()) {
                musicgo.modelo.Audio audio = gestorCatalogo.buscarAudioPorId(idCancion);
                if (audio != null) {
